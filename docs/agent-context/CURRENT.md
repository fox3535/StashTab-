...
**Branch:** `main` at `c9dcb19` (PR #32 merged); F2 provisioning record + API
deployment checkpoint on `docs/f2-provisioning-record-and-api-deploy-checkpoint`
(draft PR pending); `feature/f1-vendor-core-recovery-batch` in draft review
...
- `STASHTAB-INVENTORY-TRUTH-001` **v1.3.0** — frozen (AMENDMENT-1.3.0 on `main`
  ...
  **applied on `stashtab_staging` and reconciled/verified 2026-09-04** (D-042):
  column/index/grants only, business tables empty, cutover OFF, not deployed.
...
9. F2 slice-01 controlled receive: **merged on `main`** (D-040 local, D-041 merge).
   ...
   2026-09-04** (D-042; `CHECKPOINT-F2-SLICE-01-STAGING-PROVISIONING.md`,
   `GATES-POINTER-F2-SLICE-01.md`). Not deployed. Cutover OFF; endpoint use
   stays locked until a separate cutover unlock. API-only Railway deployment
   checkpoint prepared (`CHECKPOINT-F2-API-DEPLOYMENT-PRE-CUTOVER.md`).

## Next queued phases

1. F2 staging provisioning is **done** (column/index/grants only, verified).
   Next: a named unlock for the API-only Railway deployment
   (`CHECKPOINT-F2-API-DEPLOYMENT-PRE-CUTOVER.md`), then a **separate** cutover
   unlock. Do not use the receive endpoint until cutover is unlocked. F1
   vendor-core batch draft PR remains in review
