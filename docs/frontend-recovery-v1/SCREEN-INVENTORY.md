...
| App shell | `components/product/product-shell.tsx`, header, sidebar | Owner product chrome | Shared admin/POS frame | Layout works | No shop picker; no sign-out; Shopify nav looks live | n/a | Clerk on child routes | n/a | Sidebar IA | **adapt** into one authenticated shell with membership shop selector, explicit sign-out, and locked not-ready nav | nav, responsive, a11y |
