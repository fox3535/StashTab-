...
**Status:** `OWNER DECISIONS RECORDED — PLANNING ONLY — SUPERVISION REVISED (D-047), AWAITING REVIEW — EXECUTION NOT APPROVED — NOT EXECUTED`
...
| --- | --- | --- | --- | --- |
| `P0` Preconditions (gate, not an execution step) | Chris (owner/supervisor) + Qoder (automated operator) | Named unlock recorded; this plan approved; supervision arrangement in force (Chris supervising Qoder, D-047 — replaces the two-person rule); staging only; autodeploy off; no open incident | Unlock text names shop, actor, key, and scope | Any precondition is unverifiable (S11) |
| 1 Open the write session | Qoder (automated operator), outside the runtime role; Chris enters the credential | Create **one private, time-bounded direct session** as `stashtab_migrator`. The credential is held and entered privately by Chris, is never added to Railway or application configuration, and never appears in chat, a log, Git, or any printed output | Session open; `stashtab_truth_migrator_role` still absent from the app environment; ready `200` with `reasons: []` | The credential appears in any configuration, log, or environment value (S9, S10) |
| 2 Preflight and baseline | Qoder (automated operator; pooled `stashtab_api`, read-only) | Snapshot row counts, digest, cutover rowcount, identity counts, and column/index/grants; confirm one API process, no worker/cron, no other writer; record the freeze-window start (wall clock) | Digest equals the recorded pre-cutover baseline; cutover rowcount `0`; identity counts exactly `shops = 2` / `shop_members = 2`; bounded log window with one startup marker | Digest differs, any cutover row already exists, identity counts differ, or a second process, worker, cron, or new deployment appears (S8, S11) |
| 3 Write the generation-1 `locking` row | The step-1 migrator session (Qoder runs it only after Chris's gate-1 confirmation, D-047) | **Gate 1 (D-047): Qoder presents the verified target and step-2 baseline and pauses for Chris's explicit confirmation; silence is never approval.** Single `INSERT` for **Smoke Shop B** (`798d40f4-0832-46c4-991b-050e1310f6c4`): `generation = 1`, `status = 'locking'`, `frozen_at` set. **Do not write `complete` at this step.** | `SELECT` shows exactly one row for that shop with `generation = 1` and `status = 'locking'`, and zero rows for every other shop | More than one row, a second generation, `status = 'complete'`, or any other shop affected (S1) |
| 4 Confirm writes are still closed | Qoder (automated operator) | `GET /api/v1/ready`; unauthenticated receive; authenticated receive for **Smoke Shop B**; receive for a **different** shop | Ready `200` with `reasons: []` unchanged; unauthenticated `401`; **Smoke Shop B `503` while `locking`**; the other shop still `503` | Smoke Shop B returns anything other than `503`, ready changes, or any other shop’s gate opens (S2, S10) |
| 5 Run and record R1–R7 **while locked** | Qoder (automated operator; read-only pooled role) | Run every invariant in §5 against the locked state and record each output in the §4 audit record **before** any transition | Every invariant exactly zero variance, with R2/R3 evaluated against **zero** receive rows and R4 at its `locking` evaluation point | Any non-zero, error, timeout, or partial response — timeout is never green (S3, S4) |
| 6 Transition the same row to `complete` | The step-1 migrator session, separate transaction (Qoder runs it only after Chris's gate-2 confirmation, D-047) | **Only if all of R1–R7 are exactly zero, and after gate 2 (D-047): Qoder presents the R1–R7 results and pauses for Chris's explicit confirmation; silence is never approval.** `UPDATE` that same row to `status = 'complete'` with `opened_at` set | The row is now `generation = 1`, `status = 'complete'`, `opened_at` set, and it is still exactly one row | Any step-5 failure, a second inserted row, a second generation, or a deletion (S1, S3, S4) |
| 7 Verify cutover state | Qoder (automated operator; read-only pooled role) | Re-check `cutover_status` for Smoke Shop B, R4 at its `complete` evaluation point, ready `200` with `reasons: []`, other shops still `503`, and identity counts still `2`/`2` | `cutover_status` = `complete` for Smoke Shop B only; R4 zero variance; ready and identity counts unchanged | Any divergence from the step-2 baseline, or any other shop’s gate opening (S1, S2, S10) |
| 8 **Stop — do not receive** | Chris (owner) + Qoder (automated operator) | End the migrator session, **destroy the temporary credential/session** (the Neon `stashtab_migrator` role is **not** deleted), close the freeze window, and stop | Session closed; credential destroyed and recorded as destroyed; no `POST` to the receive endpoint; no new envelope rows | Any receive attempt — the first successful receive requires the **separate** named unlock of D-045 decision 4 |
| `R` Record (after step 8, not an execution step) | Qoder (automated operator) | Write evidence into `ACCEPTANCE-F2-SLICE-01-CONTROLLED-RECEIVE.md`, close this plan, update the gate pointer; branch + PR | Draft PR against `main`; validators green | Direct commit or push to `main` |
...
**Supervision (D-047).** The `P0` "two-person rule" precondition is replaced by
an explicit arrangement: Chris is the human owner and supervisor and privately
enters the credential; Qoder is the automated operator and is never an
independent human reviewer. Qoder pauses for Chris's explicit confirmation before
the step-3 `locking` INSERT (gate 1) and again before the step-6 `complete`
UPDATE (gate 2); a failed check stops execution and silence is never approval.
The runbook's §7 break-glass limits, §8 stop conditions, evidence preservation,
shop isolation, and the separate successful-receive unlock are unchanged. This
revision awaits review and grants no execution authority.
